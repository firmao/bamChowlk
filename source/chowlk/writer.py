    ...
    for relation_id, relation in relations.items():
        ...
        if relation["type"] == "owl:ObjectProperty":
            ...
            if relation["range"]:
                try:
                    ...
                    if blank_id in anonymous_concepts:
                        ...
                        concept_names = [
                            concepts[id]["prefix"] + ":" + concepts[id]["uri"] for id in concept_ids]
                        file.write(" ;\n")
                        file.write(
                            "\t\trdfs:range [ " + group_node["type"] + " ( \n")
            ...
            if "owl:equivalentProperty" in relation:
                file.write(" ;\n")
                file.write("\t\towl:equivalentProperty " +
    ...
    for concept_id, association in associations.items():
        ...
        for relation_id, relation in relations.items():
            ...
            if relation["type"] == "rdfs:subClassOf":
                ...
                target_name = concepts[target_id]["prefix"] + \
        ...
        for block_id, attribute_block in attribute_blocks.items():
            for attribute in attribute_block["attributes"]:
                if attribute["allValuesFrom"] and attribute["prefix"] and attribute["uri"] and attribute["datatype"]:
                    ...
                    file.write("\t\t  owl:onProperty " +
                               attribute["prefix"] + ":" + attribute["uri"] + " ;\n")
                    file.write("\t\t  owl:allValuesFrom xsd:" +
                               attribute["datatype"] + " ]")
                    ...
                    file.write("\t\t  owl:onProperty " +
                               attribute["prefix"] + ":" + attribute["uri"] + " ;\n")
                    file.write("\t\t  owl:someValuesFrom xsd:" +
                               attribute["datatype"] + " ]")
            ...
            if relation["type"] == "owl:ObjectProperty" and (relation["target"] in concepts or
                ...
                if relation["allValuesFrom"]:
                    ...
                    # Otherwise the target is an blank node of type intersection, union, etc.
                        ...
                        file.write(
                            "\t\t  owl:allValuesFrom [ " + group_node["type"] + " ( \n")
                        for name in concept_names:
                            file.write("\t\t\t\t\t\t" + name + "\n")
                    ...
                    # Otherwise the target is an blank node of type intersection, union, etc.
                        ...
                        concept_names = [
                            concepts[id]["prefix"] + ":" + concepts[id]["uri"] for id in concept_ids]
                        file.write(
                            "\t\t  owl:someValuesFrom [ " + group_node["type"] + " ( \n")
                        for name in concept_names:
                            file.write("\t\t\t\t\t\t" + name + "\n")
        ...
        for relation_id, relation in relations.items():
            ...
            if relation["type"] == "owl:disjointWith":
                ...
                if relation["target"] in concepts:
                    ...
                    complement_name = complement["prefix"] + \
                        ":" + complement["uri"]
                    if complement_name != ":":
                        file.write(
                    ...
                    file.write("\t" + relation["type"] +
                               " [ " + complement["type"] + " ( \n")
                    for id in ids:
                        concept_involved = concepts[id]["prefix"] + \
                            ":" + concepts[id]["uri"]
        ...
        for blank_id, blank in anonymous_concepts.items():
            ...
            if concept_id in blank["group"] and blank["type"] == "owl:disjointWith":
                ...
                complement_name = complement_concept["prefix"] + \
...
def write_general_axioms(file, concepts, anonymous_concepts, individuals, hexagons):
    ...
    for blank_id, blank in anonymous_concepts.items():
        if len(blank["group"]) > 2 and blank["type"] in ["owl:disjointWith"]:
            ...
            concept_names = [concepts[id]["prefix"] + ":" +
                             concepts[id]["uri"] for id in blank["group"]]
            for name in concept_names:
                file.write("\t\t" + name + "\n")
            file.write("\t\t)")
            file.write("] .")
    ...
    for hex_id, hexagon in hexagons.items():
        ...
        if len(hexagon["group"]) > 2 and hexagon["type"] in ["owl:AllDifferent"]:
            ...
            individual_names = [individuals[id]["prefix"] + ":" +
                                individuals[id]["uri"] for id in hexagon["group"]]
...
def write_triplets(file, individuals, associations, values):
    ...
    for id, association in associations.items():
        subject = association["individual"]["prefix"] + \
        ...
        for relation_id, relation in relations.items():
            ...
            target_id = relation["target"]
            object = individuals[target_id]["prefix"] + \
